...
function getUrl(url) {
  ...
  // const child = spawn('python', ['./src/scripts/downloader.py'])
  ...
  function processData(){
    if(stdoutClosed && stderrClosed) {
      const formattedFile = allFileToJson.replace(/'/g, '').replace(/\\/g, '').replace(/\\U\w{8}/g, '')
      const metadata = JSON.parse(formattedFile)
      saveTemp(metadata, videoBuffer)
      
    }
  }
  
  child.stdout.on('data', (bufferData) => {
    videoBuffer = Buffer.concat([videoBuffer, bufferData]) 
  ...
  child.stderr.on('data', (file) => {
    // console.log(file)
    allFileToJson += file.toString('latin1')
  }).on('close', () => {
    stderrClosed = true
    processData()  
  }).on('error', (err) => {
...
function saveTemp(file, videoBuffer) {
  try {
    const dadosJson = JSON.stringify(file, null, 2)
    fs.writeFile(fileTempJson, dadosJson, (err)=> err)
    fs.writeFileSync(fileTempMp4, videoBuffer)
  }catch (err) {
    logger.error(err)
  }
}
